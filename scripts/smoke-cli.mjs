...
if (
  ...
  !cliHelpOutput.includes("ilu resume") ||
...
if (
  ...
  !bashCompletionOutput.includes("help resume attach doctor bug-report update shell completion")
) {
...
if (
  !updateDryRunOutput.includes("npm install -g --ignore-scripts --install-links=true") ||
  !updateDryRunOutput.includes("i-love-urdf@latest")
) {
...
const mixedRepoReviewDir = path.join(shellDropDir, "mixed-review-repo");
fs.mkdirSync(path.join(mixedRepoReviewDir, "urdf"), { recursive: true });
fs.mkdirSync(path.join(mixedRepoReviewDir, "meshes"), { recursive: true });
fs.writeFileSync(
  path.join(mixedRepoReviewDir, "urdf", "ready.urdf"),
  "<robot name=\"ready_robot\"><link name=\"base\"/></robot>",
  "utf8"
);
fs.writeFileSync(
  path.join(mixedRepoReviewDir, "urdf", "needs-review.urdf"),
  "<robot name=\"needs_review_robot\"><link name=\"base\"><inertial><mass value=\"0\"/><origin xyz=\"0 0 0\"/><inertia ixx=\"1\" ixy=\"0\" ixz=\"0\" iyy=\"1\" iyz=\"0\" izz=\"1\"/></inertial><visual><geometry><mesh filename=\"mesh.stl\"/></geometry></visual></link></robot>",
  "utf8"
);
fs.writeFileSync(path.join(mixedRepoReviewDir, "meshes", "mesh.stl"), "solid mesh", "utf8");
...
const yUpOrientationSourcePath = path.join(root, "examples", "orientation-card", "research_wheeled_y_up.urdf");
...
if (
  ...
  invalidRepoTranscript.includes("press Enter or type /run")
) {
  throw new Error("ilu shell invalid repo recovery smoke test failed");
}
...
if (
  !invalidValidateTranscript.includes("[set] --urdf not-a-file.urdf") ||
  invalidValidateTranscript.includes("press Enter or type /run")
) {
  throw new Error("ilu shell invalid validate recovery smoke test failed");
}
...
const galleryBatchOutputDir = path.join(shellDropDir, "gallery-out");
const galleryBatchPayload = JSON.parse(
  execFileSync(
    process.execPath,
    [cliPath, "gallery-generate", "--local", multiCandidateDir, "--out", galleryBatchOutputDir],
    {
      cwd: root,
      encoding: "utf8",
      env: {
...
if (
  galleryBatchPayload.robotCount !== 2 ||
  galleryBatchPayload.generatedCount !== 2 ||
  galleryBatchPayload.needsReviewCount !== 0 ||
  !fs.existsSync(path.join(galleryBatchPayload.outputRoot, "manifest.json"))
) {
  throw new Error("ilu gallery-generate batch smoke test failed");
}

const repoFixesPayload = JSON.parse(
  execFileSync(process.execPath, [cliPath, "repo-fixes", "--local", repairableRepoDir], {
    cwd: root,
    encoding: "utf8",
  })
);
if (
  repoFixesPayload.robotCount !== 1 ||
  repoFixesPayload.generatedWithFixesCount !== 1 ||
  !Array.isArray(repoFixesPayload.sharedFixGroups) ||
  !repoFixesPayload.sharedFixGroups.some((group) => group.label === "repair mesh references")
) {
  throw new Error("ilu repo-fixes batch smoke test failed");
}

const repoFixesThenWorkOneTranscript = execFileSync(process.execPath, [cliPath, "shell"], {
  cwd: root,
  encoding: "utf8",
  env: {
    ...process.env,
...
const repoFixesReviewTranscript = execFileSync(process.execPath, [cliPath, "shell"], {
  cwd: root,
  encoding: "utf8",
  env: {
    ...process.env,
...
if (
  !repoFixesReviewTranscript.includes("review the shared repo fixes, then choose apply or not now") ||
  !repoFixesReviewTranscript.includes("can repair mesh references safely") ||
  !repoFixesReviewTranscript.includes("apply shared safe fixes across the repo now?") ||
  !repoFixesReviewTranscript.includes("repo fixes finished. gallery is ready for the repaired robots") ||
  !repoFixesReviewTranscript.includes("ready for gallery now") ||
  !repoFixesReviewTranscript.includes("review urdf/needs-review.urdf") ||
  !repoFixesReviewTranscript.includes("next /gallery for the ready robots, or /work-one to review urdf/needs-review.urdf")
) {
  throw new Error("ilu shell repo-fixes review-and-apply smoke test failed");
}
