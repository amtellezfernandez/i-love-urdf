...
import { printHelp, renderAttachHelp, renderResumeHelp } from "./commands/cliHelp";
...
import { readLatestIluSharedSession } from "./session/sharedSession";
...
const run = async () => {
  ...
  if (rawCommand === "attach") {
    if (args.has("help")) {
      console.log(renderAttachHelp());
      return;
    }

    const sessionId = positionals[0]?.trim();
    if (!sessionId) {
      cliHelpers.fail(renderAttachHelp());
    }
