...
const buildInstallSpec = (packageName: string, version: string): string => `${packageName}@${version}`;

const encodeRegistryPackageName = (packageName: string): string =>
...
const fetchLatestVersionFromNpmRegistry = async (
  metadata: InstalledPackageMetadata,
  timeoutMs: number
): Promise<string | null> => {
  const response = (await requestJson(
    `https://registry.npmjs.org/${encodeRegistryPackageName(metadata.name)}/latest`,
    timeoutMs,
...
export const checkForUpdateAvailability = async (
  ...
  const checkPromise = (async () => {
    try {
      const latestVersion = await fetchLatestVersionFromNpmRegistry(
        metadata,
