...
const magnitude = (vector) => Math.sqrt(vector[0] * vector[0] + vector[1] * vector[1] + vector[2] * vector[2]);
...
const distanceToCanonical = (vector) => {
    const canonical = [
        [1, 0, 0],
        [-1, 0, 0],
        [0, 1, 0],
    ...
    return Math.min(...canonical.map((candidate) => Math.sqrt((vector[0] - candidate[0]) * (vector[0] - candidate[0]) +
        (vector[1] - candidate[1]) * (vector[1] - candidate[1]) +
        (vector[2] - candidate[2]) * (vector[2] - candidate[2]))));
};
const countSummary = (findings) => ({
...
function healthCheckUrdf(urdfContent, options = {}) {
    ...
    const findings = [];
    const validation = (0, validateUrdf_1.validateUrdf)(urdfContent);
    ...
    Array.from(robot.querySelectorAll(":scope > link")).forEach((link) => {
        ...
        const determinant = ixx * (iyy * izz - iyz * iyz) -
            ixy * (ixy * izz - iyz * ixz) +
            ixz * (ixy * iyz - iyy * ixz);
    ...
    let orientationGuess;
    if (options.includeOrientation !== false) {
        orientationGuess = (0, guessOrientation_1.guessUrdfOrientation)(urdfContent);
        findings.push({
            level: "info",
            ...
            suggestion: orientationGuess.suggestedApplyOrientation.command ||
