...
const mujocoMeshPrep_1 = require("./mesh/mujocoMeshPrep");
const mujocoMeshPrep_2 = require("./mesh/mujocoMeshPrep");
const xmlDom_1 = require("./xmlDom");
const xacroNode_1 = require("./xacro/xacroNode");
const loadSourceNode_1 = require("./sources/loadSourceNode");
const taskFamilies_1 = require("./tasks/taskFamilies");
...
const SUPPORTED_COMMAND_SET = new Set(SUPPORTED_COMMANDS);
...
const inspectLocalMjcfMeshRisks = (urdfPath, urdfContent) => {
    ...
    for (const meshElement of Array.from(doc.querySelectorAll("mesh"))) {
        ...
        if (faceCount <= mujocoMeshPrep_1.DEFAULT_MESH_COMPRESSION_MAX_FACES)
            continue;
    ...
    const meshSummary = riskyMeshList
        ...
        .join(", ");
    return [
        `Detected ${riskyMeshList.length} STL mesh${riskyMeshList.length === 1 ? "" : "es"} above the likely MuJoCo face limit of ${mujocoMeshPrep_1.DEFAULT_MESH_COMPRESSION_MAX_FACES}. MuJoCo will likely fail to load them: ${meshSummary}.${commandHint}`,
    ];
};
const printHelp = () => {
    const familySections = taskFamilies_1.TASK_FAMILIES.map((family) => {
...
const extractMeshRefs = (urdfContent) => {
    ...
    return meshEls
        ...
        .map((ref) => (0, index_1.parseMeshReference)(ref));
};
const run = async () => {
    ...
    if (command === "inspect-repo") {
        ...
        const result = local
            ? await (0, localRepositoryInspection_1.inspectLocalRepositoryUrdfs)({ path: local }, {
                maxCandidatesToInspect,
                concurrency,
            })
            : await (() => {
                const parsed = (0, index_1.parseGitHubRepositoryReference)(github || "");
                if (!parsed) {
                ...
                const accessToken = getOptionalStringArg(args, "token") ||
                    process.env.GITHUB_TOKEN ||
                    process.env.GH_TOKEN;
                return (0, index_1.inspectGitHubRepositoryUrdfs)({
                    ...parsed,
            ...
            : github
                ? await (() => {
                    ...
                    const accessToken = getOptionalStringArg(args, "token") ||
                        process.env.GITHUB_TOKEN ||
                        process.env.GH_TOKEN;
                    return (0, xacroNode_1.expandGitHubRepositoryXacro)({
                        ...parsed,
                        path: pathOverride ?? parsed.path,
                        ref: refOverride ?? parsed.ref,
                    }, {
    ...
    if (command === "probe-xacro-runtime") {
        const result = await (0, xacroNode_1.probeXacroRuntime)({
            pythonExecutable: getOptionalStringArg(args, "python"),
            wheelPath: getOptionalStringArg(args, "wheel"),
        });
    ...
    if (command === "setup-xacro-runtime") {
        const result = await (0, xacroNode_1.setupXacroRuntime)({
            pythonExecutable: getOptionalStringArg(args, "python"),
        ...
        const result = localPath
            ...
            : await (() => {
                ...
                const accessToken = getOptionalStringArg(args, "token") ||
                    process.env.GITHUB_TOKEN ||
                    process.env.GH_TOKEN;
                return (0, loadSourceNode_1.loadSourceFromGitHub)({
                    reference: {
                        ...parsed,
        ...
        const result = local
            ...
            : await (() => {
                ...
                const accessToken = getOptionalStringArg(args, "token") ||
                    process.env.GITHUB_TOKEN ||
                    process.env.GH_TOKEN;
                return (0, index_1.repairGitHubRepositoryMeshReferences)({
                    ...parsed,
                    path: pathOverride ?? parsed.path,
                    ref: refOverride ?? parsed.ref,
                }, {
    ...
    if (command === "inspect-meshes") {
        ...
        const result = (0, mujocoMeshPrep_2.inspectMeshes)({
            meshDir,
            maxFaces,
            meshes,
            limits,
    ...
    if (command === "compress-meshes") {
        ...
        const result = (0, mujocoMeshPrep_1.compressMeshes)({
            meshDir,
            maxFaces,
            meshes,
            limits,
    ...
    if (command === "diff") {
        ...
        const result = (0, index_1.compareUrdfs)(readText(leftPath), readText(rightPath));
        console.log(JSON.stringify({ ...result, leftPath, rightPath }, null, 2));
        return;
    }
    ...
    if (command === "validate") {
        const result = (0, index_1.validateUrdf)(urdfContent);
        console.log(JSON.stringify(result, null, 2));
        return;
    }
    if (command === "health-check") {
        const result = (0, index_1.healthCheckUrdf)(urdfContent);
        console.log(JSON.stringify(result, null, 2));
    ...
    if (command === "analyze") {
        const result = (0, index_1.analyzeUrdf)(urdfContent);
        console.log(JSON.stringify(result, null, 2));
        return;
    }
    if (command === "guess-orientation") {
        ...
        const targetUpAxis = targetUpAxisRaw;
        const targetForwardAxis = targetForwardAxisRaw;
        const result = (0, index_1.guessUrdfOrientation)(urdfContent, {
            targetUpAxis,
            targetForwardAxis,
        });
        console.log(JSON.stringify(result, null, 2));
    ...
    if (command === "fix-mesh-paths") {
        ...
        const outPath = getOptionalStringArg(args, "out");
        const result = (0, index_1.fixMeshPaths)(urdfContent, packageName);
    ...
    if (command === "pretty-print") {
        ...
        console.log(JSON.stringify({ urdfContent: pretty, indent, outPath: outPath || null }, null, 2));
        return;
    }
    ...
    if (command === "set-joint-axis") {
        ...
        const result = (0, index_1.setJointAxisInUrdf)(urdfContent, jointName, xyz);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    if (command === "set-joint-type") {
        ...
        const result = (0, index_1.updateJointTypeInUrdf)(urdfContent, jointName, jointType, lowerLimit, upperLimit);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    if (command === "set-joint-limits") {
        ...
        const lowerLimit = getOptionalNumberArg(args, "lower");
        const upperLimit = getOptionalNumberArg(args, "upper");
        const result = (0, index_1.updateJointLimitsInUrdf)(urdfContent, jointName, lowerLimit, upperLimit);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    if (command === "set-joint-velocity") {
        ...
        const result = (0, index_1.updateJointVelocityInUrdf)(urdfContent, jointName, velocity);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    ...
    if (command === "rotate-90") {
        ...
        const rotated = (0, index_1.rotateRobot90Degrees)(urdfContent, axisRaw);
    ...
    if (command === "normalize-robot") {
        const outPath = getOptionalStringArg(args, "out");
        const result = (0, normalizeRobot_1.normalizeRobot)(urdfContent, {
    ...
    if (command === "reassign-joint") {
        ...
        const result = (0, index_1.updateJointLinksInUrdf)(urdfContent, jointName, parentLink, childLink);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    if (command === "set-material-color") {
        ...
        const result = (0, index_1.updateMaterialColorInUrdf)(urdfContent, linkName, materialName, colorHex);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    ...
    if (command === "rename-joint") {
        ...
        const result = (0, index_1.renameJointInUrdf)(urdfContent, oldJointName, newJointName);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
    if (command === "rename-link") {
        ...
        const result = (0, index_1.renameLinkInUrdf)(urdfContent, oldLinkName, newLinkName);
        writeOutIfRequested(outPath, result.content);
        console.log(JSON.stringify({ ...result, outPath: outPath || null }, null, 2));
        return;
    }
