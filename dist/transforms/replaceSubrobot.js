...
exports.replaceSubrobotInUrdf = void 0;
...
const sanitizePrefix = (value) => value
    .trim()
    .replace(/[^A-Za-z0-9_]+/g, "_")
    .replace(/^_+|_+$/g, "");
const normalizeComparableName = (value) => value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "");
const formatCandidateList = (names) => names.slice(0, 5).map((name) => `"${name}"`).join(", ");
const collectDescendantLinks = (rootLinkName, joints) => {
    const descendants = new Set([rootLinkName]);
    let progressed = true;
    while (progressed) {
        progressed = false;
...
const resolveName = (sourceName, prefix) => prefix ? `${prefix}__${sourceName}` : sourceName;
const buildCollisionSet = (elements) => new Set(elements.map((element) => element.getAttribute("name") || "").filter(Boolean));
const parseTripletOrZero = (value) => {
    const parsed = (value || "")
        .trim()
        .split(/\s+/)
        .filter(Boolean)
...
const getOriginAttributes = (joint) => {
...
const resolveRequestedName = (requestedName, availableNames, kind) => {
    ...
    if (caseInsensitiveMatches.length === 1) {
        return { ok: true, value: caseInsensitiveMatches[0] };
    }
    const requestedNormalized = normalizeComparableName(requestedName);
    const normalizedMatches = availableNames.filter((name) => normalizeComparableName(name) === requestedNormalized);
    if (normalizedMatches.length === 1) {
        return { ok: true, value: normalizedMatches[0] };
    }
    ...
    const suggestions = availableNames.filter((name) => {
        const candidateLower = name.toLowerCase();
        const candidateNormalized = normalizeComparableName(name);
        return (candidateNormalized.includes(requestedNormalized) ||
            requestedNormalized.includes(candidateNormalized) ||
...
const replaceSubrobotInUrdf = (hostUrdfContent, options) => {
    if (!hostUrdfContent.trim()) {
        return fail(hostUrdfContent, "No URDF content available");
    }
    const hostParsed = (0, urdfParser_1.parseURDF)(hostUrdfContent);
    if (!hostParsed.isValid) {
        return fail(hostUrdfContent, hostParsed.error || "Failed to parse host URDF");
    }
    const replacementParsed = (0, urdfParser_1.parseURDF)(options.replacementUrdfContent);
    if (!replacementParsed.isValid) {
        return fail(hostUrdfContent, replacementParsed.error || "Failed to parse replacement URDF");
    }
    ...
    const availableHostLinkNames = hostLinks
        ...
        .filter(Boolean);
    const resolvedTargetRoot = resolveRequestedName(options.targetRootLink, availableHostLinkNames, "Target root link");
    ...
    const inboundJoints = hostJoints.filter((joint) => joint.querySelector("child")?.getAttribute("link") === targetRootName);
    if (inboundJoints.length === 0) {
        return fail(hostUrdfContent, `No inbound joint found for target root "${targetRootName}"`);
    }
    ...
    const selectedInboundJoint = inboundJoints.find((joint) => {
        const parentLink = joint.querySelector("parent")?.getAttribute("link");
    ...
    if (!selectedInboundJoint) {
        return fail(hostUrdfContent, `Could not resolve mount joint for target root "${targetRootName}"`);
    }
    const preservedMountParent = selectedInboundJoint.querySelector("parent")?.getAttribute("link") || undefined;
    ...
    const removedJointElements = hostJoints.filter((joint) => {
        const parentLink = joint.querySelector("parent")?.getAttribute("link");
        const childLink = joint.querySelector("child")?.getAttribute("link");
        return (childLink === targetRootName ||
    ...
    const replacementLinkNames = replacementLinks
        .map((link) => link.getAttribute("name") || "")
        .filter(Boolean);
    const resolvedReplacementRoot = resolveRequestedName(options.replacementRootLink, replacementLinkNames, "Replacement root link");
    ...
    const replacementRoot = replacementLinks.find((link) => link.getAttribute("name") === replacementRootName);
    if (!replacementRoot) {
        return fail(hostUrdfContent, `Replacement root link "${replacementRootName}" not found`);
    }
    const replacementLinkSet = collectDescendantLinks(replacementRootName, replacementJoints);
    const importLinks = replacementLinks.filter((link) => replacementLinkSet.has(link.getAttribute("name") || ""));
    const importJoints = replacementJoints.filter((joint) => {
        const parentLink = joint.querySelector("parent")?.getAttribute("link");
        const childLink = joint.querySelector("child")?.getAttribute("link");
        return Boolean(parentLink &&
            childLink &&
            replacementLinkSet.has(parentLink) &&
            replacementLinkSet.has(childLink));
    });
    const prefix = sanitizePrefix(options.prefix || "");
    const hostLinkNames = buildCollisionSet(hostLinks.filter((link) => !removedLinkSet.has(link.getAttribute("name") || "")));
    const hostJointNames = buildCollisionSet(hostJoints.filter((joint) => !removedJoints.includes(joint.getAttribute("name") || "")));
    ...
    for (const link of importLinks) {
        ...
        if (hostLinkNames.has(mappedName)) {
            return fail(hostUrdfContent, `Imported link "${mappedName}" would collide with an existing host link. Use --prefix.`);
        }
    ...
    for (const joint of importJoints) {
        ...
        if (hostJointNames.has(mappedName)) {
            return fail(hostUrdfContent, `Imported joint "${mappedName}" would collide with an existing host joint. Use --prefix.`);
        }
    ...
    for (const material of replacementMaterials) {
        ...
        if (hostMaterialNames.has(mappedName)) {
            return fail(hostUrdfContent, `Imported material "${mappedName}" would collide with an existing host material. Use --prefix.`);
        }
    ...
    replacementMaterials.forEach((material) => {
        const cloned = material.cloneNode(true);
        const sourceName = material.getAttribute("name");
    ...
    importJoints.forEach((joint) => {
        const cloned = joint.cloneNode(true);
    ...
    const mountJointName = resolvedMountJointName ||
    ...
    child.setAttribute("link", linkMap.get(replacementRootName) || replacementRootName);
    mountJoint.appendChild(child);
    hostRobot.appendChild(mountJoint);
    return {
        success: true,
        content: (0, urdfParser_1.serializeURDF)(hostParsed.document),
        removedLinks,
        removedJoints,
...
exports.replaceSubrobotInUrdf = replaceSubrobotInUrdf;
