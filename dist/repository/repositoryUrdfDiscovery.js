...
const repositoryBasename = (repositoryPath) => {
...
const scoreRepositoryUrdfCandidate = (candidate) => {
    ...
    const parentDir = repositoryBasename((0, repositoryMeshResolution_1.repositoryDirname)(candidate.path));
    let score = 0;
    const isUrdfXacro = (0, xacroContract_1.isUrdfXacroPath)(nameLower);
    ...
    if (nameLower.includes("model"))
        score += 6;
    if (parentDir && candidateStem === parentDir)
        score += 24;
    if (hasPathSegment(candidate.path, "config"))
        score -= 25;
    if (hasPathSegment(candidate.path, "launch"))
        score -= 20;
    if (hasPathSegment(candidate.path, "test"))
        score -= 20;
    if (hasPathSegment(candidate.path, "ros2_control"))
        score -= 15;
    if (hasPathSegment(candidate.path, "module"))
        score -= 12;
    if (pathLower.includes("simulation"))
        score -= 18;
    if (pathLower.includes("pybullet"))
        score -= 12;
    ...
    if (nameLower.includes("include"))
        score -= 10;
    if (parentDir && candidateStem.startsWith(`${parentDir}_`))
        score -= 8;
    if (/_arm$|_base$|_gripper$|_head$|_end_effector$/.test(candidateStem))
        score -= 6;
    if (candidateStem === "field" || candidateStem === "world")
        score -= 30;
    if (candidateStem === "arena" || candidateStem === "stadium")
        score -= 20;
    if (candidate.isXacro && !isUrdfXacro)
        score -= 10;
    else if (candidate.isXacro)
        score -= 2;
    return score;
};
