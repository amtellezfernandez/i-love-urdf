...
exports.autoFitCollisionGeometry = void 0;
exports.computeCylinderDiagnostics = computeCylinderDiagnostics;
exports.computeSphereDiagnostics = computeSphereDiagnostics;
exports.fitCylinderPercentilePCA = fitCylinderPercentilePCA;
exports.fitCylinderConstrainedAxis = fitCylinderConstrainedAxis;
exports.computePCA = computePCA;
exports.computeRotationToAxis = computeRotationToAxis;
...
const transformVerticesToLinkFrame = (vertices, origin) => {
...
function computeCylinderDiagnostics(vertices, pca) {
    const vertexCount = vertices.length / 3;
    const axis = pca.axis;
...
function computeSphereDiagnostics(vertices, pca) {
    const vertexCount = vertices.length / 3;
    const centroid = pca.centroid;
    const [lambda1, lambda2, lambda3] = pca.eigenvalues;
    const elongation = lambda1 / Math.max(lambda2, 1e-10);
...
function fitCylinderPercentilePCA(vertices, pca, diagnostics) {
    const vertexCount = vertices.length / 3;
    const axis = pca.axis;
    const centroid = pca.centroid;
    const tValues = [];
...
function fitCylinderConstrainedAxis(vertices, minX, maxX, minY, maxY, minZ, maxZ) {
...
function computePCA(vertices) {
    ...
    return {
        axis: eigen.eigenvectors[maxEigenIndex],
...
function computeRotationToAxis(targetAxis) {
    const zAxis = [0, 0, 1];
...
const autoFitCollisionGeometry = (bounds, visualOrigin, requestedType) => {
    ...
    if (requestedType === "sphere") {
        ...
        if (diagnostics.outlierRatio > 1.3) {
            warning = warning
                ...
                : `High outlier ratio (${diagnostics.outlierRatio.toFixed(2)}) - using robust radius to ignore protrusions.`;
        }
        return {
            geometryType: "sphere",
            geometryParams: {
    ...
    if (diagnostics.elongation > 5) {
        if (diagnostics.roundness < 1.2 && diagnostics.outlierRatio < 1.2) {
            ...
            fitResult = fitCylinderConstrainedAxis(transformedVertices, minX, maxX, minY, maxY, minZ, maxZ);
...
exports.autoFitCollisionGeometry = autoFitCollisionGeometry;
