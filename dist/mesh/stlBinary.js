...
exports.inspectBinaryStlBuffer = inspectBinaryStlBuffer;
exports.inspectBinaryStlFile = inspectBinaryStlFile;
exports.readBinaryStl = readBinaryStl;
exports.readAsciiStl = readAsciiStl;
exports.computeTriangleBounds = computeTriangleBounds;
exports.readBinaryStlBounds = readBinaryStlBounds;
exports.readAsciiStlBounds = readAsciiStlBounds;
exports.readStlBounds = readStlBounds;
exports.readStlTriangles = readStlTriangles;
exports.simplifyBinaryStlTriangles = simplifyBinaryStlTriangles;
exports.chooseSimplifiedBinaryStl = chooseSimplifiedBinaryStl;
exports.writeBinaryStl = writeBinaryStl;
const fs = require("node:fs");
const STL_HEADER_BYTES = 80;
const STL_COUNT_OFFSET = STL_HEADER_BYTES;
const STL_RECORD_BYTES = 50;
const STL_MIN_EDGE = 1e-6;
const clipIndex = (value, divisions) => {
    if (value < 0)
        return 0;
    if (value >= divisions)
        return divisions - 1;
...
function inspectBinaryStlBuffer(buffer) {
...
function inspectBinaryStlFile(filePath) {
...
function readAsciiStl(filePath) {
...
function readBinaryStlBounds(filePath) {
    return computeTriangleBounds(readBinaryStl(filePath).triangles);
}
const ASCII_VERTEX_PATTERN = /vertex\s+([+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)\s+([+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)\s+([+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)/g;
function readAsciiStlBounds(filePath) {
    const content = fs.readFileSync(filePath, "utf8");
    let minX = Number.POSITIVE_INFINITY;
    let minY = Number.POSITIVE_INFINITY;
    let minZ = Number.POSITIVE_INFINITY;
...
function readStlTriangles(filePath) {
...
function simplifyBinaryStlTriangles(triangles, divisions) {
    if (divisions < 2) {
...
function chooseSimplifiedBinaryStl(triangles, maxFaces, candidateDivisions = [160, 128, 112, 96, 80, 72, 64, 56, 48, 40, 32, 24, 16, 12, 8, 6, 4, 3, 2]) {
...
function writeBinaryStl(filePath, header, triangles) {
