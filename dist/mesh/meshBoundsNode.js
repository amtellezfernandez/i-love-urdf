...
exports.readObjBounds = readObjBounds;
exports.readDaeBounds = readDaeBounds;
exports.readMeshBounds = readMeshBounds;
...
const emptyBounds = (format) => ({
    min: [0, 0, 0],
    max: [0, 0, 0],
    vertexCount: 0,
    format,
});
const updateBounds = (current, x, y, z) => {
...
function readObjBounds(filePath) {
...
const parseFloatArrayValues = (text) => text
    .trim()
    .split(/\s+/)
    .map((value) => Number(value))
    .filter((value) => Number.isFinite(value));
const getAccessorStride = (source) => {
...
function readDaeBounds(filePath) {
    const content = fs.readFileSync(filePath, "utf8");
    const document = (0, nodeDomRuntime_1.parseNodeXmlDocument)(content, "application/xml");
    ...
    const preferred = sources.filter((source) => /position/i.test(source.getAttribute("id") || "") ||
...
function readMeshBounds(filePath) {
    const extension = (0, meshFormats_1.extractExtension)(filePath)?.toLowerCase();
    if (extension === ".stl") {
        const bounds = (0, stlBinary_1.readStlBounds)(filePath);
        return {
            min: bounds.min,
            max: bounds.max,
