...
exports.summarizeRepoFixesPreviewPanel = exports.buildApplyRepoFixesSuggestion = void 0;
const formatCount = (count, singular, plural = `${singular}s`) => `${count} ${count === 1 ? singular : plural}`;
const buildApplyRepoFixesSuggestion = () => ({
...
exports.buildApplyRepoFixesSuggestion = buildApplyRepoFixesSuggestion;
const summarizeRepoFixesPreviewPanel = (repoContext) => {
    ...
    const requiredXacroCandidates = payload.candidates.filter((candidate) => (candidate.xacroArgs ?? []).some((arg) => arg.isRequired));
    const lines = [`source ${repoContext.sourceLabel}`];
...
exports.summarizeRepoFixesPreviewPanel = summarizeRepoFixesPreviewPanel;
