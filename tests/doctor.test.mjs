...
test("ilu doctor --json emits a stable diagnostics contract", () => {
  ...
  assert.equal(typeof report.ilu.name, "string");
  assert.equal(typeof report.ilu.version, "string");
  assert.equal(report.ilu.installSpec, `${report.ilu.name}@${report.ilu.version}`);
  ...
  assert.equal(typeof report.xacro.packageVersions, "object");
  assert.equal(Array.isArray(report.xacro.packageVersions), false);
});
